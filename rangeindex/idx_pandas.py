...
class PandasIndex:
    ...
    def __init__(self, fields: Dict[str, type]):
        ...
        self.df = pd.DataFrame({field: pd.Series(dtype=PYTYPE_TO_PANDAS[dtype]) for field, dtype in fields.items()})
    ...
    def add_many(self, objs: List[Any]):
        ...
        new_objs = {obj_ids[i]: objs[i] for i in range(len(obj_ids)) if obj_ids[i] not in old_obj_ids}
