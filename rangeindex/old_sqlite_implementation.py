...
class RangeIndex:
    ...
    def find_obj_ids(self, query: Optional[List[Tuple]] = None, limit=0.001) -> List:
        ...
        for i, triplet in enumerate(query):
            ...
            if i < len(query)-1:
                ...
                q.append(f'{field} {op} ?')
        limit_int = int(limit * len(self.objs))
        q.append(f'LIMIT {limit_int}')

        cur = self.conn.cursor()
        result = cur.execute('\n'.join(q), values)
        rows = list(r[0] for r in result.fetchall())

        # Check if the resulting rowcount is very large (hit the limit)
        if len(rows) == limit_int:
            # This query should not use an index.
            # Redo it with no index.
            q = [f'SELECT {PYOBJ_COL} FROM {self.table_name} NOT INDEXED WHERE']
            values = []
            for i, triplet in enumerate(query):
                field, op, value = triplet
                values.append(value)
