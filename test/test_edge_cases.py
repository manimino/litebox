...
def test_index_on_missing_attributes(backend):
    ri = RangeIndex({"z": float}, backend=backend)
    t = Thing()
    ri.add(t)
...
def test_index_namedtuple(backend):
    ...
    ri = RangeIndex({"x": float, "y": float}, backend=backend)
    ri.add(pt)
    ls = ri.find([("x", "<=", 1)])
    assert ls == [pt]
...
def test_multiple_rangeindex_instances(backend):
    t1 = Thing()
    ri1 = RangeIndex({"x": int}, backend=backend)
    ri1.add(t1)

    t2 = Thing()
    ri2 = RangeIndex({"x": int}, backend=backend)
    ri2.add(t2)
...
def test_multiple_adds(backend):
    ...
    thing_3 = Thing(x=3)
    ri = RangeIndex({"x": int}, backend=backend)
