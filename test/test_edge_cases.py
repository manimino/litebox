...
def test_index_on_missing_attributes(engine):
    ri = RangeIndex({"z": float}, engine=engine)
    t = Thing()
    ri.add(t)
...
def test_index_namedtuple(engine):
    ...
    ri = RangeIndex({"x": float, "y": float}, engine=engine)
    ri.add(pt)
    ls = ri.find([("x", "<=", 1)])
    assert ls == [pt]
...
def test_multiple_rangeindex_instances(engine):
    t1 = Thing()
    ri1 = RangeIndex({"x": int}, engine=engine)
    ri1.add(t1)

    t2 = Thing()
    ri2 = RangeIndex({"x": int}, engine=engine)
    ri2.add(t2)
...
def test_multiple_adds(engine):
    ...
    thing_3 = Thing(x=3)
    ri = RangeIndex({"x": int}, engine=engine)
...
def test_find_on_empty_idx(engine):
    ri = RangeIndex({'x': float}, engine=engine)
